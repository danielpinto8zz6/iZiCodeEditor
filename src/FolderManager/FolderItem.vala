...
namespace iZiCodeEditor.FolderManager {
    ...
    internal class FolderItem : Item {
        ...
        protected Gtk.MenuItem create_submenu_for_new () {
            var new_folder_item = new Gtk.MenuItem.with_label ("Folder");
            new_folder_item.activate.connect(() => add_folder ());

            var new_file_item = new Gtk.MenuItem.with_label ("Empty File");
            ...
            var new_item = new Gtk.MenuItem.with_label ("New");
        ...
        protected void add_folder () {
            ...
            var new_folder = file.file.get_child ("untitled folder");

            var n = 1;
            while (new_folder.query_exists ()) {
                new_folder = file.file.get_child ("untitled folder %d".printf (n));
                n++;
            }
        ...
        protected void add_file () {
            ...
            var new_file = file.file.get_child ("new file");

            var n = 1;
            while (new_file.query_exists ()) {
                new_file = file.file.get_child ("new file %d".printf (n));
                n++;
            }
